...
- SpaceToDepth accepts static rank-4 FP32 inputs when a positive block size
  ...
  The unoptimized CREStereo convex-upsampling form is recognized only when its
...
- Conv is limited to static FP32 shapes. Conv1D is represented as Conv2D with
  ...
  required; downstream Conv restrictions still apply. YOLOv6s
  ConvTranspose2D, MeloTTS decoder ConvTranspose1D, and the degenerate
  ConvTranspose3D paths in unoptimized FlyingThings are validated end to end.
...
- Slice requires compile-time starts and ends; axes and steps must be omitted
  ...
  Resize
  supports static rank-4 spatial scaling in either the YOLOv8
  nearest/asymmetric/floor configuration, the Candy
  nearest/half_pixel/round_prefer_floor configuration, or the Paddle
  linear/half_pixel and Distill-Any-Depth linear/align_corners configurations.
...
- GatherElements requires static FP32 data/result and i32/i64 indices at ranks
  ...
  an execution-time error. Rank-3/4/5 feature and weight cases, including the
  FastVLM runtime-index path, are covered.
- ScatterElements is currently restricted to the identity-index form used by
  Gemma 3: static equal-shape FP32 tensors, constant indices that select every
...
- GatherND likewise requires static FP32 data/result ranks 1-5 and static-shape
  ...
  FlatBuffer size.
- InstanceNormalization is supported through the importer's
  LayerNormalization form exercised by Candy: static rank-4 FP32, spatial
...
- RMSLayerNormalization supports static rank-2 through rank-5 FP32 last-axis
  ...
  covers Gemma's importer-recomposed residual Add. The importer recomposition
