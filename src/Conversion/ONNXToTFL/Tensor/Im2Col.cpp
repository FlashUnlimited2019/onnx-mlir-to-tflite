...
public:
  ...
  LogicalResult matchAndRewrite(ONNXIm2ColOp op, OpAdaptor adaptor,
    ...
    int64_t outputHeight = inputShape[2] + pads[0] + pads[2] - kernel[0] + 1;
    int64_t outputWidth = inputShape[3] + pads[1] + pads[3] - kernel[1] + 1;
    ...
    Value logicalInput =
        createTFLOperation(rewriter, op.getLoc(), "tfl.transpose",
            TypeRange{inputType}, ValueRange{adaptor.getX(), permutation})
            ->getResult(0);
      ...
      for (int64_t x = 0; x < kernel[1]; ++x) {
        ...
        patches.push_back(
            createTFLOperation(rewriter, op.getLoc(), "tfl.reshape",
                TypeRange{expandedPatchType}, ValueRange{patch, expandedShape})
                ->getResult(0));
      }
    }
