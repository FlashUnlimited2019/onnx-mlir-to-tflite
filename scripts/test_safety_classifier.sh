...
MODEL="${1:-${REPO_ROOT}/../models/safety_classifier.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/safety_classifier.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "Safety-classifier model not found: ${MODEL}" >&2
  exit 1
fi
