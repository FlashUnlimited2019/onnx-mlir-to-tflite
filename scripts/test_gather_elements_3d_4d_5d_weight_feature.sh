...
MODEL="${1:-${REPO_ROOT}/../models/gather_elements_3d_4d_5d_weight_feature.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/gather_elements_3d_4d_5d_weight_feature.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "GatherElements rank-3/4/5 model not found: ${MODEL}" >&2
  exit 1
fi
