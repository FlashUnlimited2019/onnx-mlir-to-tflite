...
MODEL="${1:-${REPO_ROOT}/../models/many_greater_broadcast_2.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/many_greater_broadcast_2.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "Greater rank-cases model not found: ${MODEL}" >&2
  exit 1
fi
