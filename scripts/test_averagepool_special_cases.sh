...
MODEL="${1:-${REPO_ROOT}/../models/averagepool_special_cases.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/averagepool_special_cases.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "AveragePool special-cases model not found: ${MODEL}" >&2
  exit 1
fi
