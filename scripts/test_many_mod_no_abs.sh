...
MODEL="${1:-${REPO_ROOT}/../models/many_mod_no_abs.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/many_mod_no_abs.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "Mod rank-cases model not found: ${MODEL}" >&2
  exit 1
fi
