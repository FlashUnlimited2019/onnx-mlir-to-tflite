...
MODEL="${1:-${REPO_ROOT}/../models/FastVLM-0.5B_vision_encoder.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/FastVLM-0.5B_vision_encoder.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "FastVLM vision encoder not found: ${MODEL}" >&2
  exit 1
fi
