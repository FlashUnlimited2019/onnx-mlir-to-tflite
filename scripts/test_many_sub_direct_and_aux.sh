...
MODEL="${1:-${REPO_ROOT}/../models/many_sub_direct_and_aux.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/many_sub_direct_and_aux.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "Sub direct/auxiliary model not found: ${MODEL}" >&2
  exit 1
fi
