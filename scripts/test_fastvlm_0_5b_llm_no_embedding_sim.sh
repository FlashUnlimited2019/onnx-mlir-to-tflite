...
MODEL_DIR="${REPO_ROOT}/../models/FastVLM-0.5B-llm-no-embedding_sim"
MODEL="${1:-${MODEL_DIR}/FastVLM-0.5B-llm-no-embedding_sim.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/FastVLM-0.5B-llm-no-embedding_sim.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "FastVLM LLM model not found: ${MODEL}" >&2
  exit 1
fi
if [[ ! -f "${MODEL}.data" ]]; then
  echo "FastVLM LLM external data not found: ${MODEL}.data" >&2
  exit 1
fi
