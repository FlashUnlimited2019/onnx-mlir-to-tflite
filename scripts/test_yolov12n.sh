...
MODEL="${1:-${REPO_ROOT}/../models/yolov12n_640.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/yolov12n_640.tflite}"
PYTHON_BIN="${PYTHON:-python}"

if [[ ! -f "${MODEL}" ]]; then
  echo "YOLOv12n model not found: ${MODEL}" >&2
  exit 1
fi
