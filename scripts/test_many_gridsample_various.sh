...
MODEL="${1:-${REPO_ROOT}/../models/many_gridsample_various.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/many_gridsample_various.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "GridSample various model not found: ${MODEL}" >&2
  exit 1
fi
