...
MODEL="${1:-${REPO_ROOT}/../models/many_equal.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/many_equal.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "Equal rank-cases model not found: ${MODEL}" >&2
  exit 1
fi
