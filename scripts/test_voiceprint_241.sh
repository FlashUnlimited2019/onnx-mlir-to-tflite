...
MODEL="${1:-${REPO_ROOT}/../models/voiceprint_241.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/voiceprint_241.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "Voiceprint model not found: ${MODEL}" >&2
  exit 1
fi
