...
MODEL="${1:-${REPO_ROOT}/../models/retinaface-640.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/retinaface-640.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "RetinaFace model not found: ${MODEL}" >&2
  exit 1
fi
