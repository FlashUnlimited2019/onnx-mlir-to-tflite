...
MODEL="${1:-${REPO_ROOT}/../models/shufflenet-v2-224.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/shufflenet-v2-224.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "ShuffleNet V2 model not found: ${MODEL}" >&2
  exit 1
fi
