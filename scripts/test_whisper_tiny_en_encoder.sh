...
MODEL="${1:-${REPO_ROOT}/../models/whisper_tiny_en_encoder.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/whisper_tiny_en_encoder.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "Whisper tiny English encoder model not found: ${MODEL}" >&2
  exit 1
fi
