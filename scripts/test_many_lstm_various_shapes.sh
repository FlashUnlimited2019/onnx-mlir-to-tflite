...
MODEL="${1:-${REPO_ROOT}/../models/many_lstm_various_shapes.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/many_lstm_various_shapes.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "LSTM various-shapes model not found: ${MODEL}" >&2
  exit 1
fi
