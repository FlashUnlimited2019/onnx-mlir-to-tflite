...
MODEL="${1:-${REPO_ROOT}/../models/paddle_human_seg.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/paddle_human_seg.tflite}"
PYTHON_BIN="${PYTHON:-python}"

if [[ ! -f "${MODEL}" ]]; then
  echo "Paddle human-segmentation model not found: ${MODEL}" >&2
  exit 1
fi
