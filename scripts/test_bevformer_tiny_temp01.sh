...
MODEL="${1:-${REPO_ROOT}/../models/bevformer_tiny_temp01.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/bevformer_tiny_temp01.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "BEVFormer tiny model not found: ${MODEL}" >&2
  exit 1
fi
