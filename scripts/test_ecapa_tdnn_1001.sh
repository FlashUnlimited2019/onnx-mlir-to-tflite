...
MODEL="${1:-${REPO_ROOT}/../models/ECAPA-TDNN_pretrain_no_fbank_1001_sim.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/ECAPA-TDNN_pretrain_no_fbank_1001_sim.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "ECAPA-TDNN model not found: ${MODEL}" >&2
  exit 1
fi
