...
MODEL="${1:-${REPO_ROOT}/../models/many_averagepool_3d4d5d.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/many_averagepool_3d4d5d.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "AveragePool rank-cases model not found: ${MODEL}" >&2
  exit 1
fi
