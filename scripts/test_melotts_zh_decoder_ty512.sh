...
MODEL="${1:-${REPO_ROOT}/../models/melotts_zh_decoder_ty512.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/melotts_zh_decoder_ty512.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "MeloTTS Chinese decoder model not found: ${MODEL}" >&2
  exit 1
fi
