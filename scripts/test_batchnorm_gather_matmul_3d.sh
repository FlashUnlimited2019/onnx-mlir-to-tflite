...
MODEL="${1:-${REPO_ROOT}/../models/batchnorm_3d_4d_5d_gatherelements_matmul_output3d.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/batchnorm_3d_4d_5d_gatherelements_matmul_output3d.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "BatchNorm/GatherElements/MatMul model not found: ${MODEL}" >&2
  exit 1
fi
