...
MODEL="${1:-${REPO_ROOT}/../models/many_reducel2_various_shapes_static_single_io.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/many_reducel2_various_shapes_static_single_io.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "ReduceL2 rank-cases model not found: ${MODEL}" >&2
  exit 1
fi
