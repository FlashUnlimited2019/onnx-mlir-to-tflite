...
MODEL="${1:-${REPO_ROOT}/../models/LightStereo-M.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/LightStereo-M.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "LightStereo-M model not found: ${MODEL}" >&2
  exit 1
fi
