...
MODEL="${1:-${REPO_ROOT}/../models/matmul_many_cases_1.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/matmul_many_cases_1.tflite}"
PYTHON_BIN="${PYTHON:-python}"

if [[ ! -f "${MODEL}" ]]; then
  echo "MatMul many-cases model not found: ${MODEL}" >&2
  exit 1
fi
