...
MODEL="${1:-${REPO_ROOT}/../models/yolox-m-640.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/yolox-m-640.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "YOLOX-M model not found: ${MODEL}" >&2
  exit 1
fi
