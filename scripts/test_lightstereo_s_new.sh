...
MODEL="${1:-${REPO_ROOT}/../models/LightStereo-S_new.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/LightStereo-S_new.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "LightStereo-S model not found: ${MODEL}" >&2
  exit 1
fi
