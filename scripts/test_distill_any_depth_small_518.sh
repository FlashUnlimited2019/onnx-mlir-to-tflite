...
MODEL="${1:-${REPO_ROOT}/../models/distill_any_depth_small_518.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/distill_any_depth_small_518.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "Distill-Any-Depth model not found: ${MODEL}" >&2
  exit 1
fi
