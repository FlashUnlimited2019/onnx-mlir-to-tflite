...
MODEL="${1:-${REPO_ROOT}/../models/many_gathernd_int64_constant_indices.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/many_gathernd_int64_constant_indices.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "GatherND constant-indices model not found: ${MODEL}" >&2
  exit 1
fi
