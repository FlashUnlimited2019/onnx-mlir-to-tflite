...
MODEL="${1:-${REPO_ROOT}/../models/single_io_add_broadcast_cases.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/single_io_add_broadcast_cases.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "Add broadcast-cases model not found: ${MODEL}" >&2
  exit 1
fi
