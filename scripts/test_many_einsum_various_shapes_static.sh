...
MODEL="${1:-${REPO_ROOT}/../models/many_einsum_various_shapes_static.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/many_einsum_various_shapes_static.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "Einsum static-shapes model not found: ${MODEL}" >&2
  exit 1
fi
