...
MODEL="${1:-${REPO_ROOT}/../models/many_argmax_2_3_4_5d.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/many_argmax_2_3_4_5d.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "ArgMax many-cases model not found: ${MODEL}" >&2
  exit 1
fi
