...
MODEL="${1:-${REPO_ROOT}/../models/mstcn_6-20-60_3000.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/mstcn_6-20-60_3000.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "MSTCN model not found: ${MODEL}" >&2
  exit 1
fi
...
TF_CPP_MIN_LOG_LEVEL=3 "${PYTHON_CMD[@]}" \
  ...
  --max-operators 1050 \
  --max-constant-tensors 420 \
  --forbid-op CUSTOM \
  --forbid-op Flex \
  --forbid-op SPACE_TO_BATCH_ND \
  --forbid-op BATCH_TO_SPACE_ND
TF_CPP_MIN_LOG_LEVEL=3 "${PYTHON_CMD[@]}" \
