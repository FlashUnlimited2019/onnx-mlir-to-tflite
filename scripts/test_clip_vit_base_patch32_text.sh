...
MODEL="${1:-${REPO_ROOT}/../models/clip-vit-base-patch32_text_model.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/clip-vit-base-patch32_text_model.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "CLIP text model not found: ${MODEL}" >&2
  exit 1
fi
...
for SEED in 20260806 20260807; do
  TF_CPP_MIN_LOG_LEVEL=3 TF_ENABLE_ONEDNN_OPTS=0 "${PYTHON_CMD[@]}" \
    ...
    --seed "${SEED}" \
    --num-threads 8 \
    --integer-input-range input_ids:0:49408 \
    --min-cosine 0.999999 \
