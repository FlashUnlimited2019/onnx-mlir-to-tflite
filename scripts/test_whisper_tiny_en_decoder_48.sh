...
MODEL="${1:-${REPO_ROOT}/../models/whisper_tiny_en_decoder_48.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/whisper_tiny_en_decoder_48.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "Whisper tiny English decoder model not found: ${MODEL}" >&2
  exit 1
fi
...
for SEED_AND_STDDEV in 20260806:0.5 20260807:2.0; do
  ...
  TF_CPP_MIN_LOG_LEVEL=3 TF_ENABLE_ONEDNN_OPTS=0 "${PYTHON_CMD[@]}" \
    ...
    --stddev "${STDDEV}" \
    --num-threads 8 \
    --integer-input-range input_ids:0:51864 \
    --min-cosine 0.999999 \
