...
MODEL="${1:-${REPO_ROOT}/../models/many_gridsample_bilinear_nearest_static_single_io.onnx}"
OUTPUT="${2:-${REPO_ROOT}/build/test/models/many_gridsample_bilinear_nearest_static_single_io.tflite}"

if [[ ! -f "${MODEL}" ]]; then
  echo "GridSample cases model not found: ${MODEL}" >&2
  exit 1
fi
